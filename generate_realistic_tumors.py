...
def create_realistic_tumor(size=256, tumor_type='brain'):
    ...
    # Use the ellipse as base mask
    mask = ellipse.copy()
    
    # Add some irregularity by randomly eroding/dilating different parts
    mask = ndimage.binary_dilation(mask, iterations=np.random.randint(2, 5))
    mask = ndimage.binary_erosion(mask, iterations=np.random.randint(1, 3))
    
    # Fill any holes and smooth
    ...
    # DEBUG: Check mask
    print(f"DEBUG: Mask has {mask_smooth.sum()} tumor pixels out of {mask_smooth.size} total pixels")
    print(f"DEBUG: Mask dtype: {mask_smooth.dtype}, unique values: {np.unique(mask_smooth)}")
